...
**生成时间**: 2026/01/13 19:49:11 (2026-01-13T11:49:11.275Z)
**执行耗时**: 1.39 秒
...
- ✅ safeAsync 错误处理 应该使用自定义错误映射器 (0ms)
- ✅ safeAsync 错误处理 应该调用 onError 回调 (5ms)
- ✅ safeAsync 错误处理 onError 应该在抛出错误前调用 (0ms)
- ✅ safeAsync 错误处理 应该正确处理 AppError (1ms)
- ✅ safeAsync 错误处理 应该正确处理 DOMException AbortError (0ms)
- ✅ safeAsync 组合功能 应该同时支持超时和重试 (33ms)
- ✅ safeAsync 组合功能 应该同时支持超时和回退值 (12ms)
- ✅ safeAsync 组合功能 应该同时支持取消和超时 (101ms)
- ✅ safeAsync 边界情况 应该处理没有 signal 和 timeout 的情况 (0ms)
...
- ✅ createTimeoutSignal 超时行为 应该精确控制超时时间 (52ms)
...
- ✅ withAsyncCatch 成功情况 应该保持原始函数的类型 (0ms)
...
- ✅ withAsyncCatch 实际使用场景 应该用于 API 调用 (1ms)
- ✅ withAsyncCatch 实际使用场景 应该用于数据库操作 (0ms)

### tests/safeAll.spec.ts

- ✅ safeAll 成功情况 应该返回所有结果当所有 Promise 成功时 (0ms)
...
- ✅ asyncTry 类型泛型 应该支持不同的返回类型 (0ms)
- ✅ asyncTry 异步行为 应该等待 Promise 完成 (12ms)
- ✅ asyncTry 异步行为 应该正确处理延迟的错误 (12ms)

### tests/AppError.spec.ts

- ✅ AppError 基本功能 应该创建 AppError 实例 (1ms)
...
- ✅ defaultErrorMapper 未知错误处理 应该将 null 转换为 AppError (0ms)
- ✅ defaultErrorMapper 未知错误处理 应该将 undefined 转换为 AppError (0ms)
- ✅ defaultErrorMapper 错误类型识别 应该正确识别 AppError (0ms)
- ✅ defaultErrorMapper 错误类型识别 应该正确识别 DOMException AbortError (1ms)
...
- ✅ mergeAbortSignals 取消行为 应该处理已取消的 signal (1ms)
- ✅ mergeAbortSignals 取消行为 应该处理所有 signal 都已取消的情况 (1ms)
...
- ✅ mergeAbortSignals 边界情况 应该处理大量 signal (0ms)
...
- ✅ ErrorCode 类型定义 ErrorCodeType 应该包含所有错误代码 (0ms)
- ✅ ErrorCode 类型定义 应该可以用于类型注解 (1ms)
